# Project Plan - Avvo Site "Find a Lawyer" (Business Lawyers, location-specific e.g., El Segundo, CA)

## Overview
- Objective: deliver an Avvo Site "Find a Lawyer" business-lawyer directory/profile experience for a given location (e.g., El Segundo, CA) with lead capture (call/message/chat), Pro CTAs, sponsored listings, transparency/insights content, and strong SEO/accessibility.
...
- Phase 5 - Quality & Compliance (approx 2 weeks): Unit/contract/E2E tests mapped to US IDs; performance budgets (LCP/CLS/TTI); security review (CSRF/XSS/input validation); accessibility audit; mobile/responsive verification; multi-platform/browser testing (Chrome, Safari, Firefox, Edge) across key breakpoints; analytics validation. Demo: test/QA report; release: incremental fixes to staging/prod behind flags.
...
- BE Engineer (2): owns search endpoints, lead/chat integrations, sponsored logic, auth/save; pairs with FE on contracts.
...
- Cross-browser/platform matrix: primary browsers (Chrome, Safari, Firefox, Edge) on desktop; mobile/tablet breakpoints on iOS/Android; prioritized scenarios (search/sort/pagination, CTAs, chat, message intake, save/bookmark).
- CI/testing: unit test coverage target >=80% enforced in CI; contract/API tests in CI; E2E smoke on critical paths (search/sort/pagination, contact/chat, message intake, save/bookmark, sponsored clicks) on every PR and before release.
